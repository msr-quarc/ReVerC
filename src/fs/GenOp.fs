...
/// <summary>
/// Takes a quotation and compiles it to a circuit.
/// </summary>
/// <param name="prog">Program quotation</param>
let compile program =
  // Parsing
  ...
  match unify_eq top' eqs bnds [] with
    | None -> failwith <| sprintf "Error: could not infer types\n"
    | Some subs -> 
        let gexp' = applySubs subs gexp
        // Compilation
        match compileGCCirc (gexp', circGCInit) with
          | Err s              -> failwith <| sprintf "%s\n" s
          | Val (output, circ) -> (typ, output, circ)
...
/// </summary>
(*
let printQSharp typ output circ =
    let top   = ref 0
    let newID () =
        top := !top + 1
        "var_" + (int (newID ()))
