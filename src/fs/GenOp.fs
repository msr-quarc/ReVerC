...
let generateGraph gexp (cleanup : CleanupType) = 
    ...
    // Main generator
    let rec genGraph_rec gexp nodes ctx dep = match gexp with
      | LET (x, t1, t2) -> maybe {
        let! (rval1, nodes', _, dep') = genGraph_rec t1 nodes ctx dep
        let! (rval2, nodes'', _, dep'') = genGraph_rec t2 nodes' (x, rval)::ctx dep'
        return (genGraph_rec t2 nodes'' ((x, rval)::ctx) dep'')
        }
      | LAMBDA (x, t) -> Some (rFun x t, nodes, ctx, dep)
      | APPLY (t1, t2) -> maybe {
        let! (rval1, nodes', _, dep') = genGraph_rec t1 nodes ctx dep
        let! (rval2, nodes'', _, dep'') = genGraph_rec t2 nodes' ctx dep'
        match rval1 with
          | rFun (x, t) -> 
              let! (rval3, nodes''', _, dep''') = genGraph_rec t nodes'' (x, t2)::ctx dep''
              return (rval3, nodes''', ctx, dep''')
          | _ -> ignore <| printf "Apply: LHS is not a lambda: \n"; show tm
        }
      | XOR _ | AND _ | EQ _ | Not _ -> genBexp_rec gexp nodes ctx dep
      | VAR x -> maybe {
        let! rval = lookup ctx x
        return (rval, nodes, ctx, dep)
        }
      //| SET_VAR (x, t) -> maybe {
      //  let! (rval, nodes', _, dep') = genGraph_rec t nodes' ctx dep'
      //  return (Unit, nodes'
      //  }
      | GET_ARRAY (t, i) -> maybe {
        let! (rval, nodes', _, dep') = genGraph_rec t nodes ctx dep
        match rval with
          | rArray arr when i < Array.length arr ->
            return (rBool arr.[i], nodes', ctx, dep')
      ...
      | SEQUENCE (t1, t2) -> maybe {
        let! (rval1, nodes', _, dep') = genGraph_rec t1 nodes ctx dep
        let! (rval2, nodes'', _, dep'') = genGraph_rec t2 nodes' ctx dep''
        return (rval2, nodes'', ctx, dep'')
      | ARRAY tlst -> maybe {
        let f (acc, nodes, dep) t = match genGraph_rec (t, nodes, ctx, dep) with
          | None -> None
          | Some (rval, nodes', _, dep') -> Some (rval::acc, nodes', dep')
        let! (vlst, nodes', dep') = foldM f ([], nodes, dep) tlst

      | APPEND (t1,t2) -> maybe {
        let! (rval1, nodes', _, dep') = genGraph_rec t1 nodes ctx dep
        let! (rval2, nodes'', _, dep'') = genGraph_rec t2 nodes' ctx dep'
        match (rval1, rval2) with
          | (rArray arr1, rArray arr2) ->
            return (rArray (Array.append arr1 arr2), nodes'', ctx, dep'')
    ...
    let rec rScope gexp (varMap : Map<string * int option, int>) defMap appList = 
        ...
        let doBoolExp expr appList = 
            let rec getBExp appList = 
                ...
                | NOT x ->
                    let x', applist' = getBExp appList x
                    BNot x', applist'
                | x -> 
        ...
        let rec gather_params lst exp = match exp with
          | LAMBDA(s, t) -> gather_params (lst@[s]) t
          | _ -> (lst, exp)
        let rec gather_inputs exp lst = match exp with
          | VAR s -> (s, lst)
          | APPLY(t1, t2) -> gather_inputs t1 (t2::lst)
          | _ -> failwith <| sprintf "Unsupported application"
        ...
        | LET(name, LAMBDA(var, exp), next) -> 
            let (vars, exp) = gather_params [] (LAMBDA(var, exp))
            let vMap' = Map.ofList << (List.zip <| List.map (fun x -> x, None) (List.rev vars)) <| (List.rev [ 0..[var].Length - 1 ])
            let _, defMap', appList' = rScope exp vMap' defMap <| List.map RVAR (List.rev [var])
        ...
        // f a b
        | APPLY(t1, t2) ->
            let (s, inputs) = gather_inputs (APPLY(t1, t2)) []
